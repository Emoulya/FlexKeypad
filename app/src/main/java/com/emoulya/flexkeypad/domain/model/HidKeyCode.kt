package com.emoulya.flexkeypad.domain.model

data class KeyDefinition(
    val code: Int,
    val label: String,
    val category: KeyCategory,
    val description: String = ""
)

enum class KeyCategory(val displayName: String) {
    MODIFIERS("Modifiers"),
    LETTERS("Letters"),
    NUMBERS("Numbers"),
    FUNCTION("Function Keys"),
    NAVIGATION("Navigation"),
    CONTROL("Control & Editing"),
    NUMPAD("Numpad")
}

object HidKeyCodes {
    // USB HID Keyboard / Keypad Page (0x07) Keycodes
    const val KEY_NONE = 0x00

    // Modifiers (0xE0 - 0xE7)
    const val KEY_LEFT_CTRL = 0xE0
    const val KEY_LEFT_SHIFT = 0xE1
    const val KEY_LEFT_ALT = 0xE2
    const val KEY_LEFT_GUI = 0xE3
    const val KEY_RIGHT_CTRL = 0xE4
    const val KEY_RIGHT_SHIFT = 0xE5
    const val KEY_RIGHT_ALT = 0xE6
    const val KEY_RIGHT_GUI = 0xE7

    // Letters A-Z (0x04 - 0x1D)
    const val KEY_A = 0x04
    const val KEY_B = 0x05
    const val KEY_C = 0x06
    const val KEY_D = 0x07
    const val KEY_E = 0x08
    const val KEY_F = 0x09
    const val KEY_G = 0x0A
    const val KEY_H = 0x0B
    const val KEY_I = 0x0C
    const val KEY_J = 0x0D
    const val KEY_K = 0x0E
    const val KEY_L = 0x0F
    const val KEY_M = 0x10
    const val KEY_N = 0x11
    const val KEY_O = 0x12
    const val KEY_P = 0x13
    const val KEY_Q = 0x14
    const val KEY_R = 0x15
    const val KEY_S = 0x16
    const val KEY_T = 0x17
    const val KEY_U = 0x18
    const val KEY_V = 0x19
    const val KEY_W = 0x1A
    const val KEY_X = 0x1B
    const val KEY_Y = 0x1C
    const val KEY_Z = 0x1D

    // Numbers 1-9, 0 (0x1E - 0x27)
    const val KEY_1 = 0x1E
    const val KEY_2 = 0x1F
    const val KEY_3 = 0x20
    const val KEY_4 = 0x21
    const val KEY_5 = 0x22
    const val KEY_6 = 0x23
    const val KEY_7 = 0x24
    const val KEY_8 = 0x25
    const val KEY_9 = 0x26
    const val KEY_0 = 0x27

    // Common Controls
    const val KEY_ENTER = 0x28
    const val KEY_ESCAPE = 0x29
    const val KEY_BACKSPACE = 0x2A
    const val KEY_TAB = 0x2B
    const val KEY_SPACE = 0x2C
    const val KEY_MINUS = 0x2D
    const val KEY_EQUAL = 0x2E
    const val KEY_LEFT_BRACKET = 0x2F
    const val KEY_RIGHT_BRACKET = 0x30
    const val KEY_BACKSLASH = 0x31
    const val KEY_SEMICOLON = 0x33
    const val KEY_QUOTE = 0x34
    const val KEY_GRAVE = 0x35
    const val KEY_COMMA = 0x36
    const val KEY_PERIOD = 0x37
    const val KEY_SLASH = 0x38
    const val KEY_CAPS_LOCK = 0x39

    // Function Keys F1-F12 (0x3A - 0x45)
    const val KEY_F1 = 0x3A
    const val KEY_F2 = 0x3B
    const val KEY_F3 = 0x3C
    const val KEY_F4 = 0x3D
    const val KEY_F5 = 0x3E
    const val KEY_F6 = 0x3F
    const val KEY_F7 = 0x40
    const val KEY_F8 = 0x41
    const val KEY_F9 = 0x42
    const val KEY_F10 = 0x43
    const val KEY_F11 = 0x44
    const val KEY_F12 = 0x45

    // Navigation & Editing
    const val KEY_PRINT_SCREEN = 0x46
    const val KEY_SCROLL_LOCK = 0x47
    const val KEY_PAUSE = 0x48
    const val KEY_INSERT = 0x49
    const val KEY_HOME = 0x4A
    const val KEY_PAGE_UP = 0x4B
    const val KEY_DELETE = 0x4C
    const val KEY_END = 0x4D
    const val KEY_PAGE_DOWN = 0x4E
    const val KEY_RIGHT_ARROW = 0x4F
    const val KEY_LEFT_ARROW = 0x50
    const val KEY_DOWN_ARROW = 0x51
    const val KEY_UP_ARROW = 0x52

    // Keypad (Numpad)
    const val KEY_NUM_LOCK = 0x53
    const val KEY_KP_SLASH = 0x54
    const val KEY_KP_ASTERISK = 0x55
    const val KEY_KP_MINUS = 0x56
    const val KEY_KP_PLUS = 0x57
    const val KEY_KP_ENTER = 0x58
    const val KEY_KP_1 = 0x59
    const val KEY_KP_2 = 0x5A
    const val KEY_KP_3 = 0x5B
    const val KEY_KP_4 = 0x5C
    const val KEY_KP_5 = 0x5D
    const val KEY_KP_6 = 0x5E
    const val KEY_KP_7 = 0x5F
    const val KEY_KP_8 = 0x60
    const val KEY_KP_9 = 0x61
    const val KEY_KP_0 = 0x62
    const val KEY_KP_DOT = 0x63

    val ALL_KEYS: List<KeyDefinition> = buildList {
        // Modifiers (as standalone assignable keys)
        add(KeyDefinition(KEY_LEFT_CTRL, "Ctrl", KeyCategory.MODIFIERS, "Left Control"))
        add(KeyDefinition(KEY_LEFT_ALT, "Alt", KeyCategory.MODIFIERS, "Left Alt"))
        add(KeyDefinition(KEY_LEFT_SHIFT, "Shift", KeyCategory.MODIFIERS, "Left Shift"))
        add(KeyDefinition(KEY_LEFT_GUI, "Win", KeyCategory.MODIFIERS, "Windows / GUI"))
        add(KeyDefinition(KEY_RIGHT_CTRL, "R-Ctrl", KeyCategory.MODIFIERS, "Right Control"))
        add(KeyDefinition(KEY_RIGHT_ALT, "R-Alt", KeyCategory.MODIFIERS, "Right Alt"))
        add(KeyDefinition(KEY_RIGHT_SHIFT, "R-Shift", KeyCategory.MODIFIERS, "Right Shift"))
        add(KeyDefinition(KEY_RIGHT_GUI, "R-Win", KeyCategory.MODIFIERS, "Right Windows / GUI"))

        // Letters
        val letters = ('A'..'Z').toList()
        for (i in letters.indices) {
            add(KeyDefinition(KEY_A + i, letters[i].toString(), KeyCategory.LETTERS))
        }

        // Numbers
        add(KeyDefinition(KEY_1, "1", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_2, "2", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_3, "3", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_4, "4", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_5, "5", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_6, "6", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_7, "7", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_8, "8", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_9, "9", KeyCategory.NUMBERS))
        add(KeyDefinition(KEY_0, "0", KeyCategory.NUMBERS))

        // Function Keys
        for (i in 1..12) {
            add(KeyDefinition(KEY_F1 + (i - 1), "F$i", KeyCategory.FUNCTION))
        }

        // Controls
        add(KeyDefinition(KEY_ENTER, "Enter", KeyCategory.CONTROL))
        add(KeyDefinition(KEY_ESCAPE, "Esc", KeyCategory.CONTROL))
        add(KeyDefinition(KEY_BACKSPACE, "Backspace", KeyCategory.CONTROL))
        add(KeyDefinition(KEY_TAB, "Tab", KeyCategory.CONTROL))
        add(KeyDefinition(KEY_SPACE, "Space", KeyCategory.CONTROL))
        add(KeyDefinition(KEY_DELETE, "Delete", KeyCategory.CONTROL))
        add(KeyDefinition(KEY_CAPS_LOCK, "Caps Lock", KeyCategory.CONTROL))

        // Navigation
        add(KeyDefinition(KEY_UP_ARROW, "Up Arrow", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_DOWN_ARROW, "Down Arrow", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_LEFT_ARROW, "Left Arrow", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_RIGHT_ARROW, "Right Arrow", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_HOME, "Home", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_END, "End", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_PAGE_UP, "Page Up", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_PAGE_DOWN, "Page Down", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_INSERT, "Insert", KeyCategory.NAVIGATION))
        add(KeyDefinition(KEY_PRINT_SCREEN, "PrtSc", KeyCategory.NAVIGATION))

        // Numpad
        add(KeyDefinition(KEY_NUM_LOCK, "Num Lock", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_SLASH, "KP /", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_ASTERISK, "KP *", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_MINUS, "KP -", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_PLUS, "KP +", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_ENTER, "KP Enter", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_7, "KP 7", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_8, "KP 8", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_9, "KP 9", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_4, "KP 4", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_5, "KP 5", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_6, "KP 6", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_1, "KP 1", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_2, "KP 2", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_3, "KP 3", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_0, "KP 0", KeyCategory.NUMPAD))
        add(KeyDefinition(KEY_KP_DOT, "KP .", KeyCategory.NUMPAD))
    }

    fun getKeyLabel(code: Int): String {
        return ALL_KEYS.find { it.code == code }?.label ?: if (code == KEY_NONE) "None" else "Key $code"
    }
}
